...
| **Invalid Command**                                                                            | You are not following the exact command format of the command word that was entered.{::nomarkdown} Missing some compulsory parameters. <br> The correct format would be listed after this error message for convenience.{:/}                   |
...
| **URLs should be a valid link, and it should not be blank**                                    | The URL provided should be in one of the following formats (either `https` or `http` is fine){::nomarkdown}: <ul><li>`https://www.example.com`</li> <li>`https://example.com`</li> <li>`www.example.com`</li> <li>`example.com`</li> </ul>{:/} |
| **DateTime should be formatted as dd-MM-yyyy HHmm; e.g. 30-04-2022 1400**                      | The dateTime given is invalid. {::nomarkdown} <br/> <ul><li>Giving dates that do not exist 29th of February</li> <li>Giving time that does not exist like 2500 hours.</li></ul>  {:/}                                                          |
...
| **At least one field to edit must be provided.**                                               | No fields to edit were supplied together with the edit command.{::nomarkdown}<br/> As for `medit`, since there is currently only 1 field, it is compulsory as stated in the features section in this User Guide.{:/}                           |
